...
public class GeneralConfigManager {
	...
	protected boolean economyAsync, isBroadcastingSkillups, isBroadcastingLevelups, payInCreative, payExploringWhenFlying, addXpPlayer, hideJobsWithoutPermission, payNearSpawner, modifyChat, saveOnDisconnect, MultiServerCompatability;

	public CMISound soundLevelup;
	public CMISound soundTitleChange;

	public String modifyChatPrefix, modifyChatSuffix, modifyChatSeparator, ServerAccountName, ServertaxesAccountName, localeString = "";
	...
	public boolean ignoreOreGenerators, useBlockProtection, useNewBlockProtection, useNewExploration, useBlockProtectionBlockTracker, enableSchedule, PayForRenaming, PayForEnchantingOnAnvil, PayForEachCraft, SignsEnabled,
			SignsColorizeJobName, ShowToplistInScoreboard, useGlobalTimer, useGlobalBreakTimer, useSilkTouchProtection, UseCustomNames, PreventSlimeSplit, PreventMagmaCubeSplit, PreventHopperFillUps, PreventBrewingStandFillUps,
			informOnPaymentDisable, BrowseUseNewLook, payExploringWhenGliding = false, resetExploringData = false, disablePaymentIfMaxLevelReached, disablePaymentIfRiding, boostedItemsInOffHand = false, boostedItemsInMainHand,
			boostedArmorItems, boostedItemsSlotSpecific, multiplyBoostedExtraValues, addPermissionBoost, highestPermissionBoost, payItemDurabilityLoss, applyToNegativeIncome, useMinimumOveralPayment, useMinimumOveralPoints,
			useMinimumOveralExp, useBreederFinder, CancelCowMilking, fixAtMaxLevel, TitleChangeChat, TitleChangeActionBar, LevelChangeChat, LevelChangeActionBar, SoundLevelupUse, SoundTitleChangeUse, UseServerAccount,
	...
	/**
	 * Function to check if jobs should be hidden to players that lack permission to
	 * join the job
	 * 
	 * @return
	 */
	...
	private void loadGeneralSettings() {
		...
		c.header(Arrays.asList("General configuration.", "  The general configuration for the jobs plugin mostly includes how often the plugin", "  saves user data (when the user is in the game), the storage method, whether",
				"  to broadcast a message to the server when a user goes up a skill level.", "  It also allows admins to set the maximum number of jobs a player can have at", "  any one time."));
		c.copyDefaults(true);
		...
		c.addComment("save-on-disconnect", "Should player data be saved on disconnect?", "Player data is always periodically auto-saved and autosaved during a clean shutdown.",
		...
		c.addComment("boost-persistence", "Should job boosts persist across server restarts?", "When enabled, boosts applied via /jobs boost command will be saved and restored after server restart.",
				"This ensures that timed boosts continue for their full duration even if the server restarts.", "Set to false if this feature causes issues with your server setup.");
		...
		c.addComment("MultiServerCompatability", "Enable if you are using one data base for multiple servers across bungee network",
				"This will force to load players data every time he is logging in to have most up to date data instead of having preloaded data", "This will enable automatically save-on-disconnect feature");
		...
		c.addComment("Optimizations.NewVersion", "When set to true staff will be informed about new Jobs plugin version", "You need to have jobs.versioncheck permission node");
		ShowNewVersion = c.get("Optimizations.NewVersion", true);

		c.addComment("Optimizations.InformDuplicates", "Can be used to check if your database contains any duplicated user entries.");
		InformDuplicates = c.get("Optimizations.InformDuplicates", false);

		c.addComment("Optimizations.DecimalPlaces.Money", "Decimal places to be shown");
		...
		c.addComment("Optimizations.DBCleaning.Jobs.Use", "Warning!!! before enabling this feature, please make data base backup, just in case there will be some issues with data base cleaning",
				"When set to true, jobs data base will be cleaned on each startup to avoid having not used jobs", "keep in mind that this will only clean actual jobs, but not recorded players");
		...
		c.addComment("Optimizations.DBCleaning.Users.Use", "Warning!!! before enabling this feature, please make data base backup, just in case there will be some issues with data base cleaning",
		...
		c.addComment("Optimizations.AutoJobJoin.Use", "Use or not auto join jobs feature", "If you are not using auto join feature, keep it disabled");
		...
		c.addComment("Optimizations.AutoJobJoin.Delay", "Delay in seconds to perform auto join job if used after player joins server",
				"If you using offline server, try to keep it slightly more than your login plugin gives time to enter password", "For player to auto join job add permission node jobs.autojoin.[jobname]", "Op players are ignored");
		...
		c.addComment("Optimizations.DisabledWorlds.Use", "By setting this to true, Jobs plugin will be disabled in given worlds", "Only commands can be performed from disabled worlds with jobs.disabledworld.commands permission node");
		DisabledWorldsUse = c.get("Optimizations.DisabledWorlds.Use", false);
		c.addComment("Optimizations.DisabledWorlds.UseAsWhiteList", "If true, will changes the list behavior, so if a world is added to list", "the payments will only works in the given worlds.");
		...
		if (Version.isCurrentEqualOrHigher(Version.v1_16_R1)) {
			c.addComment("Optimizations.Explore.NewMethod", "Do you want to use new exploration tracking method. Only for 1.16+ servers");
		...
		c.addComment("Optimizations.Explore.SaveIntoDatabase", "While enabled explored chunk data will be saved into database and will persist over server restarts",
		...
		c.addComment("Logging.Use", "With this set to true all players jobs actions will be logged to database for easy to see statistics", "This is still in development and in future it will expand");
		...
		c.addComment("DailyQuests.ResetTime", "Defines time in 24hour format when we want to give out new daily quests", "Any daily quests given before reset will be invalid and new ones will be given out");
		...
		c.addComment("JobExpirationTime", "Fire players if their work time has expired at a job.", "Setting time to 0, will not works.", "For this to work, the player needs to get a new job for the timer to start.", "Counting in hours");
		...
		c.addComment("enable-pay-near-spawner", "Option to allow payment to be made when killing mobs from a spawner.",
		...
		c.addComment("highest-permission-boost", "Default (false) and we will prioritize specific permission over general, which means that jobs.boost.miner.money.0.5 will override and be used instead of jobs.boost.all.money.0.8",
				"When enabled we will pick highest permission node from all accesible ones", "Only applies while add-permission-boost set to false");
		...
		/*
		 * c.addComment("prevent-crop-resize-payment",
		...
		c.addComment("allow-pay-for-durability-loss.WhiteListedItems", "What items (tools) are whitelisted the player get paid, when this item has durability loss?", "Enchantments are supported, usage:", "itemName=ENCHANTMENT_NAME-level");
		List<String> tempList = c.get("allow-pay-for-durability-loss.WhiteListedItems", Arrays.asList("wooden_pickaxe=DURABILITY-1", "fishing_rod"));
		...
		c.addComment("UseCustomNames", "Do you want to use custom item/block/mob/enchant/color names?", "With this set to true names like Stone:1 will be translated to Granite", "Name list is in TranslatableWords.yml file");
		...
		c.addComment("Economy.PaymentMethods", "By disabling one of these, players no longer will get particular payment.", "Useful for removing particular payment method without editing whole jobConfig file");

		for (CurrencyType one : CurrencyType.values()) {
			one.setEnabled(c.get("Economy.PaymentMethods." + one.getName(), true));
		}

		c.addComment("Economy.GeneralMulti", "Can be used to change payment amounts for all jobs and all actions if you want to readjust them", "Amounts are in percentage, above 0 will increase payments",
				"Amount belove 0 will decrease payments", "If action pays negative amount, then value above 0 will increase that negative value",
				...
				"If you are getting paid 10 for placing wood, then same value of 50 for money income, will result in you getting 15 bucks", "This only effects base income value");
		for (CurrencyType one : CurrencyType.values()) {
			generalMulti.put(one, c.get("Economy.GeneralMulti." + one.name(), 0D) / 100D);
		}

		c.addComment("Economy.ApplyToNegativeIncome", "When set to true income which is belove 0 will get bonus applied to it",
				"In example, if you would loose 5 bucks for placing diamond block, with 100% payment bonus, that penalty disappears", "When this left at false penalty for action will remain unchanged");
		applyToNegativeIncome = c.get("Economy.ApplyToNegativeIncome", false);

		c.addComment("Economy.MinimumOveralPayment.use", "Determines minimum payment.", "In example if player uses McMMO treefeller and earns only 20%, but at same time player gets 25% penalty from dynamic payment.",
				"The player can 'get' negative amount of money", "This will limit it to particular percentage", "Works only when original payment is above 0");
		...
		c.addComment("Economy.MinimumOveralPoints.use", "Determines minimum payment for points.", "In example if player uses McMMO treefeller and earns only 20%, but at same time player gets 25% penalty from dynamic payment.",
				"The player can 'get' negative amount of points", "This will limit it to particular percentage", "Works only when original payment is above 0");
		...
		c.addComment("Economy.MinimumOveralExp.use", "Determines minimum payment for experience.", "In example if player uses McMMO treefeller and earns only 20%, but at same time player gets 25% penalty from dynamic payment.",
				"The player can 'get' negative amount of experience", "This will limit it to particular percentage", "Works only when original payment is above 0");
		...
		c.addComment("Economy.DynamicPayment.equation", "totalworkers: The total number of players on the server who have jobs", "totaljobs: The number of jobs that are enabled",
				"jobstotalplayers: The number of people in that particular job", "Exponential equation: totalworkers / totaljobs / jobstotalplayers - 1", "Linear equation: ((totalworkers / totaljobs) - jobstotalplayers)/10.0");
		try {
		...
		c.addComment("Economy.Taxes.TakeFromPlayersPayment", "With this true, taxes will be taken from players payment and he will get less money than its shown in jobs info",
		...
		c.addComment("Economy.Limit.Money", "Money gain limit", "With this enabled, players will be limited how much they can make in defined time", "Time in seconds: 60 = 1 min, 3600 = 1 hour, 86400 = 24 hours");
		...
		c.addComment("Economy.Limit.Money.MoneyLimit", "Equation to calculate max limit. Option to use total level to include players total amount levels of current jobs", "You can always use simple number to set money limit",
				"Default equation is: 500+500*(totallevel/100), this will add 1% from 500 for each level player have", "So player with 2 jobs with level 15 and 22 will have 685 limit");
		try {
		...
		c.addComment("Economy.Limit.Money.AnnouncementDelay", "Delay between announcements about reached money limit", "Keep this from 30 to 5 min (300), as players can get annoyed of constant message displaying");
		...
		limit.setStopWith(list);
		c.addComment("Economy.Limit.Point.Limit", "Equation to calculate max limit. Option to use total level to include players total amount levels of current jobs", "You can always use simple number to set limit",
				"Default equation is: 500+500*(totallevel/100), this will add 1% from 500 for each level player have", "So player with 2 jobs with level 15 and 22 will have 685 limit");
		try {
		...
		c.addComment("Economy.Limit.Point.AnnouncementDelay", "Delay between announcements about reached limit", "Keep this from 30 to 5 min (300), as players can get annoyed of constant message displaying");
		...
		limit = new CurrencyLimit();
		list = new ArrayList<>();
		c.addComment("Economy.Limit.Exp", "Exp gain limit", "With this enabled, players will be limited how much they can get in defined time", "Time in seconds: 60 = 1 min, 3600 = 1 hour, 86400 = 24 hours");
		...
		if (c.get("Economy.Limit.Exp.StopWithPoint", false))
			list.add(CurrencyType.POINTS);
		limit.setStopWith(list);
		c.addComment("Economy.Limit.Exp.Limit", "Equation to calculate max money limit. Option to use total level to include players total amount of current jobs", "You can always use simple number to set exp limit",
				"Default equation is: 5000+5000*(totallevel/100), this will add 1% from 5000 for each level player have", "So player with 2 jobs with level 15 and 22 will have 6850 limit");
		try {
		...
		c.addComment("Economy.Limit.Exp.ResetTime", "Time in 24 hour format when limit should reset. This will override TimeLimit if defined", "Example: '00:00:00' will reset timer at midnight, '04:30:00' will reset at 4:30 in the morning",
				"Set to empty field if you want to use TimeLimit");
		limit.setResetsAt(c.get("Economy.Limit.Exp.ResetTime", ""));

		c.addComment("Economy.Limit.Exp.AnnouncementDelay", "Delay between announcements about reached Exp limit", "Keep this from 30 to 5 min (300), as players can get annoyed of constant message displaying");
		limit.setAnnouncementDelay(c.get("Economy.Limit.Exp.AnnouncementDelay", 30));
		currencyLimitUse.put(CurrencyType.EXP, limit);

		c.addComment("Economy.Repair.PayForRenaming", "Do you want to give money for only renaming items in anvil?", "Players will get full pay as they would for remaining two items when they only rename one",
		...
		c.addComment("Economy.Crafting.PayForEachCraft", "With this true, player will get money for all crafted items instead of each crafting action (like with old payment mechanic)",
		...
		c.addComment("Economy.Crafting.PayForCombiningItems", "true - if players tries to combine (repair) 2 same items, like armor or weapon " + "do they get income for it?");
		payForCombiningItems = c.get("Economy.Crafting.PayForCombiningItems", true);

		c.addComment("Economy.MilkingCow.CancelMilking", "With this true, when timer is still going, cow milking event will be canceled", "With this false, player will get bucket of milk, but still no payment");
		CancelCowMilking = c.get("Economy.MilkingCow.CancelMilking", false);
		c.addComment("Economy.MilkingCow.Timer", "How ofter player can milk cows in seconds. Keep in mind that by default player can milk cow indefinitely and as often as he wants", "Set to 0 if you want to disable timer");
		CowMilkingTimer = c.get("Economy.MilkingCow.Timer", 30) * 1000;

		BlockTypes.anyToReasign = false;

		c.addComment("ExploitProtections.Furnaces.Reassign", "When enabled, players interacted furnaces will be saved into a file and will be reassigned after restart to keep giving out money",
		...
		c.addComment("ExploitProtections.Furnaces.MaxDefaultAvailable", "Defines max available furnaces each player can have to get paid from", "This can be overridden with jobs.maxfurnaces.[amount] permission node");
		...
		c.addComment("ExploitProtections.BrewingStands.Reassign", "When enabled, players interacted brewing stands will be saved into file and will be reassigned after restart to keep giving out money",
		...
		c.addComment("ExploitProtections.BrewingStands.MaxDefaultAvailable", "Defines max available brewing stands each player can have to get paid from", "Set to 0 if you want to disable this limitation",
		...
		c.addComment("ExploitProtections.General.PlaceAndBreak.Enabled", "Enable blocks protection, like ore, from exploiting by placing and destroying same block again and again.",
		...
		if (Version.isCurrentEqualOrHigher(Version.v1_14_R1)) {
			c.addComment("ExploitProtections.General.PlaceAndBreak.NewMethod", "Should we use new block protection method",
					"In most cases this is more efficient way to check for break/place protection and doesn't involve any cache or data saving into database", "Only works with 1.14+ servers");
			useNewBlockProtection = c.get("ExploitProtections.General.PlaceAndBreak.NewMethod", true);
		}

		c.addComment("ExploitProtections.General.PlaceAndBreak.BlockTracker.Enabled", "Should we use BlockTracker plugin instead of built in block tracker");
		useBlockProtectionBlockTracker = c.get("ExploitProtections.General.PlaceAndBreak.BlockTracker.Enabled", false);

		c.addComment("ExploitProtections.General.PlaceAndBreak.ProtectionOfOreGenerators", "When enabled blocks will keep protection after next generator cycle preventing quick money making aproach",
				"Keep in mind that timers are based on block protection setup");
		ignoreOreGenerators = c.get("ExploitProtections.General.PlaceAndBreak.ProtectionOfOreGenerators", true);

		c.addComment("ExploitProtections.General.PlaceAndBreak.KeepDataFor", "Only applies when old method is used", "For how long in days to keep block protection data in data base",
				"This will clean block data which ones have -1 as cooldown value", "Data base cleanup will be performed on each server startup", "This cant be more then 14 days");
		...
		/*
		 * c.addComment("ExploitProtections.General.AllowBreakPaymentForOreGenerators",
		 * "It allows you to pay for ore generator plugins continuously" +
		 * " once you have broken the block in one place.");
		 * allowBreakPaymentForOreGenerators =
		 * c.get("ExploitProtections.General.AllowBreakPaymentForOreGenerators", false);
		 */
		...
		c.addComment("ExploitProtections.General.PlaceAndBreak.GlobalBlockTimer.Place.Timer", "Time in seconds. This can only be positive number and no higher than 900",
		...
		c.addComment("ExploitProtections.General.PlaceAndBreak.GlobalBlockTimer.Break.Timer", "Time in seconds. This can only be positive number and no higher than 60",
		...
		c.addComment("ExploitProtections.General.PlaceAndBreak.SilkTouchProtection", "Enable silk touch protection.", "With this enabled players wont get paid for broken blocks from restrictedblocks list with silk touch tool.");
		...
		c.addComment("ExploitProtections.McMMO.TreeFellerMultiplier", "Players will get part of money from cutting trees with treefeller ability enabled.", "0.2 means 20% of original price");
		TreeFellerMultiplier = c.get("ExploitProtections.McMMO.TreeFellerMultiplier", 0.2);
		c.addComment("ExploitProtections.McMMO.gigaDrillMultiplier", "Players will get part of money from braking blocks with gigaDrill ability enabled.", "0.2 means 20% of original price");
		gigaDrillMultiplier = c.get("ExploitProtections.McMMO.gigaDrillMultiplier", 0.2);
		c.addComment("ExploitProtections.McMMO.superBreakerMultiplier", "Players will get part of money from braking blocks with super breaker ability enabled.", "0.2 means 20% of original price");
		...
		c.addComment("ExploitProtections.CustomFishing", "CustomFishing plugin support (Optional)", "If setting is enabled, Fish and PyroFishingPro actions are disabled and only CustomFishing action is enabled.",
		...
		if (Version.isCurrentLower(Version.v1_14_R1)) {
			c.addComment("ExploitProtections.Spawner.PreventSlimeSplit", "Prevent slime splitting when they are from spawner", "Protects against exploiting as new splitted slimes is treated as naturally spawned and not from spawner");
		...
		c.addComment("ExploitProtections.Smelt.PreventMagmaCubeSplit", "Prevent payments when hoppers moving items into brewing stands", "Player will not get paid, but items will be brewd as they supose too");
		...
		c.addComment("use-breeder-finder", "Breeder finder.", "If you are not using breeding payment, you can disable this to save little resources. Really little.");
		useBreederFinder = c.get("use-breeder-finder", true);

		c.addComment("old-job", "Old job save", "Players can leave job and return later with some level loss during that", "You can fix players level if his job level is at max level");
		levelLossPercentage = c.get("old-job.level-loss-percentage", 30);
		fixAtMaxLevel = c.get("old-job.fix-at-max-level", true);
		c.addComment("old-job.level-loss-from-max-level", "Percentage to loose when leaving job at max level", "Only works when fix-at-max-level is set to false");
		levelLossPercentageFromMax = c.get("old-job.level-loss-from-max-level", levelLossPercentage);

		c.addComment("ChatText.Messages.DefaultState", "States of chat text messages when payment is issued", "Valid options: Off, Batched", "This will be used if player disables action bar payment messages");
		...
		c.addComment("ActionBars.Messages.KeepFor", "Time in seconds action bar will remain visible if rapid state is enabled", "This time is used to define for how long we will accumulate payments to be shown in action bar",
		...
		c.addComment("Sounds", "Extra sounds on some events", "All sounds can be found in https://hub.spigotmc.org/javadocs/spigot/org/bukkit/Sound.html");
		SoundLevelupUse = c.get("Sounds.LevelUp.use", true);
		String SoundLevelupSound = c.get("Sounds.LevelUp.sound", Version.isCurrentLower(Version.v1_9_R1) ? "LEVEL_UP" : Version.isCurrentEqualOrHigher(Version.v1_21_R3) ? "ENTITY.PLAYER.LEVELUP" : "ENTITY_PLAYER_LEVELUP").toUpperCase();
		...
		String SoundTitleChangeSound = c.get("Sounds.TitleChange.sound", Version.isCurrentLower(Version.v1_9_R1) ? "LEVEL_UP" : Version.isCurrentEqualOrHigher(Version.v1_21_R3) ? "ENTITY.PLAYER.LEVELUP" : "ENTITY_PLAYER_LEVELUP")
				.toUpperCase();
		...
		c.addComment("Fireworks.LevelUp.Random", "Makes the firework to randomize, such as random colors, type, power and so on.", "These are under settings will not be work, when this enabled.");
		...
		c.addComment("Fireworks.LevelUp.type", "Firework type", "All types can be found in https://hub.spigotmc.org/javadocs/bukkit/org/bukkit/FireworkEffect.Type.html");
		...
		if (!UseRandom) {
			...
			fireworkEffect = FireworkEffect.builder().flicker(useFlicker).trail(useTrail).with(fwType).withColor(colors).withFade(colors).build();
		}
		...
		c.addComment("Signs.InfoUpdateInterval", "This is interval in sec in which signs will be updated. This is not continues update, signs are updated only on levelup, job leave, job join or similar action.");
		c.addComment("Signs.InfoUpdateInterval", "This is update for same job signs, to avoid huge lag if you have bunch of same type signs. Keep it from 1 to as many sec you want");
		...
		c.addComment("JobsGUI.SwitcheButtons", "With true left mouse button will join job and right will show more info.", "With false left mouse button will show more info, right will join job or inversely.",
		...
		c.addComment("JobsGUI.InfoButton.Slot", "Only works if you have AddEdge enabled", "Slot for info button. Set it to 0 if you want to disable it", "Locale can be customized in locale file under gui->infoLore section");
		...
		c.addComment("Commands.Jobs", "What should happen when performing clean /jobs command", "Options:", "Default - behaves like before and will open jobs help page with list of commands player can access",
		...
		c.addComment("Commands.JobsLeave.UsePerPermissionLeave", "Defines how job leave works.", "If this is true, then the user must have another permission to leave the job. jobs.command.leave.jobName");
